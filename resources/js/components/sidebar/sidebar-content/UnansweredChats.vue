...
import { useUnansweredChatsQuery as useQuery } from '@queries'
...
onMounted(() => {console.log('onMounted 2', );
  // watch(projectID, (id) => {console.log('watch 2', );
  setTimeout(function () {
    echo.private(`chats.${projectID.value}.unanswered`).listen(".chatEvent", (data) => {
...
onUnmounted(() => {
  echo.leave(`chats.${projectID.value}.unanswered`);
});
</script>
